...
'''
思路： . 表示可以是任意字符，* 表示前面的字符可以出现任意次，所以在patten中*不可能位于首位的
      aaa  a.a  ab*ac*a(b出现0次，c出现0次，这样就是aaa了)

伪代码:1. 两个字符串都为空，返回true
 2. 当第一个字符串不空，而第二个字符串空了，返回 false，如果第一个字符串空了，第二个字符串非空，需要判断。
 3. pattern下一个字符为‘*’或不 为‘*’，4. 不为‘*’,直接匹配当前字符。如果匹配成功，继续匹配下一个;如果匹配失败，直接返回 false 5. 为‘*’时，
 稍微复杂一些，因为‘*’可以代表0个或多个。a>当‘*’匹配0个字符时，str当前字符 不变，pattern当前字符后移两位，跳过这个‘*’符号;6. 当‘*’匹配1个或多个时，
 str当前字符移向 下一个，pattern当前字符不变。(这里匹配1个或多个可以看成一种情况，因为:当匹配一个 时，由于str移到了下一个字符，
 而pattern字符不变，就回到了上边的情况a;当匹配多于一个字 符时，相当于从str的下一个字符继续开始匹配)
26ms
5628k
'''
