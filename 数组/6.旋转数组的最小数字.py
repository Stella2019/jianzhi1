...
伪代码:二分法的变种 1. 设置low，high，mid表示收尾和中间数，
2. 比较low和high的数，如果 low数小于high，直接返回low数 
3. 比较low和mid的数，如果mid数大于low数(递增)， low=mid+1 
4. 重新得到mid，比较mid和high，如果mid小于high数(同递增)，high=mid 
5. 重 复第234步，直到low>=high，返回 low数就是最小值
注意的点:遇顺序数组，用二分，2. 左边有序取另一半，右边有序右边取最小，这个是和二分查 找不一样的地方，(判断条件改变)
